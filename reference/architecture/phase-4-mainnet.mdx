...
<Warning>
  This is the **blueprint** for T3, not final source. Specific
  parameter values (fee bps, bond ratios, timelock duration, signer
  composition) may shift during audit and pre-launch operational
  review. The shape (multisig admin + arbiter, timelock, fee on
  unlock, per-route bond config, USDC-only launch) is locked.
</Warning>
...
What changes vs [Phase 3](/reference/architecture/phase-3-agents-disputes):
