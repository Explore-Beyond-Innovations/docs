...
<Note>
  🔨 **Planned for T2.** Designed, not yet shipped. Builds on Phase 2's
  BLS-only auth model. Lands the agent-account model and dispute-handling
  paths on testnet, plus the multi-key `BLSKeyRegistry` upgrade.
</Note>

<Warning>
  This is the **blueprint** for T2, not final source. Field names, API
  signatures, and storage layout may shift during implementation and
  audit. The shape (custom-account agents, multi-key registry, bonded
  dispute lifecycle, deadline-based cancel paths) is locked.
</Warning>
...
<img src="/reference/architecture/_diagrams/phase-3-agents-disputes.png" alt="Phase 3 architecture — Agents and disputes" />

## Delta from Phase 2

What changes vs [Phase 2](/reference/architecture/phase-2-bls-auth):
...
<CardGroup cols={2}>
  ...
  <Card title="`ArbiterRole` actor" icon="user-shield">
    ...
    [Phase 4](/reference/architecture/phase-4-mainnet).
  </Card>
</CardGroup>
...
The standalone Noir BLS auth circuit shipped in [Phase 2](/reference/architecture/phase-2-bls-auth)
