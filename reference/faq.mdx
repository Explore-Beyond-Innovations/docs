...
<AccordionGroup>
  <Accordion title="Is ProofBridge live on mainnet?">
    Not yet. ProofBridge is currently deployed on **Ethereum Sepolia** and **Hedera Testnet** only. These are public testnets, so you can explore the full bridging experience without using real funds. Mainnet deployment is on the roadmap and will follow a thorough security review process.
  </Accordion>
  ...
  <Accordion title="What wallets are supported?">
    ProofBridge supports any EVM-compatible wallet for Ethereum Sepolia and Hedera Testnet interactions, including:
    ...
    For Stellar network interactions, standard Stellar wallets are supported. Make sure your wallet is connected to the correct testnet network before creating orders or ads.
  </Accordion>
  ...
  <Accordion title="Can I bridge any token?">
    ...
    |---|---|---|
    | ETH / WETH | Ethereum Sepolia | Hedera Testnet |
    | HBAR / WHBAR | Hedera Testnet | Ethereum Sepolia |
    | ProofBridge Token | Ethereum Sepolia ↔ Hedera Testnet | Both directions |

    Native tokens (ETH, HBAR) are automatically wrapped to their ERC-20 equivalents (WETH, WHBAR) when deposited, and unwrapped when released.
  </Accordion>
  ...
  <Accordion title="What is the Maker's risk?">
    ...
    **Relayer liveness**: In the current Phase 1 design, the relayer coordinates settlement. If the relayer is offline, settlement may be delayed even after both parties have deposited.
  ...
  <Accordion title="What chains will be supported in the future?">
    ...
    - **Phase 1 (current)**: Ethereum Sepolia and Hedera Testnet (EVM), with Stellar Testnet (Soroban)
    - **Phase 5**: Non-EVM chains including Starknet and Solana, along with monitoring, anomaly detection, and safe-mode controls
