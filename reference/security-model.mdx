...
<Note>
  The EIP-712 domain for ProofBridge uses `name = "Proofbridge"` and `version = "1"`. Chain IDs and contract addresses are part of the Order struct, not the domain — making every field inspectable in your wallet's signing prompt.
</Note>
...
<AccordionGroup>
  <Accordion title="Phase 1 — Pre-authorization relayer (current)">
  ...
  <Accordion title="Phase 2 — BLS aggregation (coming soon)">
  ...
  <Accordion title="Phase 4 — Stateless permissionless relayers">
...
| Multi-relayer liveness | Stateless relayer design | No (Phase 4) |
