...
Before you can bridge tokens or provide liquidity, you need to connect a wallet. ProofBridge supports EVM-compatible wallets (such as MetaMask) for Ethereum Sepolia and Hedera Testnet, and Stellar-native wallets for the Stellar network. Authentication uses Sign-In With Ethereum (SIWE) — you sign a message to prove wallet ownership, not a transaction, so no gas is spent during login.

<Note>
  ProofBridge is live on testnet. Make sure your wallet has testnet funds before connecting. Get Sepolia ETH from the [Sepolia faucet](https://sepoliafaucet.com) and HBAR from the [Hedera faucet](https://portal.hedera.com).
</Note>
...
<Tabs>
  <Tab title="EVM (MetaMask / WalletConnect)">
    Use this flow for Ethereum Sepolia or Hedera Testnet.
    ...
    <Steps>
      ...
      <Step title="Switch to the correct network">
        If your wallet is not already on Ethereum Sepolia or Hedera Testnet, ProofBridge prompts you to switch. Click **Switch network** in the prompt and approve the network change in your wallet.
      </Step>
