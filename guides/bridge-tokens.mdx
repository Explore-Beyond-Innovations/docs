...
<Note>
  ProofBridge currently operates on testnet (Ethereum Sepolia, Hedera Testnet, and Stellar). Use testnet funds only. Get Sepolia ETH from the [Sepolia faucet](https://sepoliafaucet.com) and HBAR from the [Hedera portal](https://portal.hedera.com).
</Note>
...
<Steps>
  ...
  <Step title="Enter the transfer amount and recipient address">
    ...
    <Note>
      If you are sending native ETH or HBAR, it is automatically wrapped to WETH or WHBAR on deposit. You do not need to wrap tokens manually before bridging.
    </Note>
  </Step>
