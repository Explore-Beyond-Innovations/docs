...
<Note>
  Since v2, the proof is only half of the authority chain: settlement also requires **both counterparties' aggregated BLS signature**, verified on-chain against the key registry — and the Merkle roots the proof is checked against are the very roots both parties [co-signed](/v2/concepts/co-signing). The relayer initiates nothing on its own authority; it aggregates, proves, and submits what the two traders authorized.
</Note>
