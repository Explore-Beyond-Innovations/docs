...
- **Testnet gas.** You need Sepolia ETH (for EVM) or Stellar Testnet XLM (for Soroban) to cover transaction fees. Get Sepolia ETH from a public faucet, or Stellar Testnet XLM from [friendbot](https://friendbot.stellar.org/).
- **Testnet bridge tokens.** After you connect your wallet, open the in-app [Faucet](https://app.pfbridge.xyz/faucet) to claim the ERC-20 / SAC / SEP-41 tokens that ProofBridge currently supports. These are the tokens you'll actually move between chains — XLM and native ETH aren't bridgeable on their own.
