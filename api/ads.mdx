...
Liquidity advertisements (ads) are the core primitive of the ProofBridge marketplace. A Maker creates an ad to signal that they are willing to provide liquidity on a specific cross-chain route — for example, locking USDC on Hedera so bridgers can receive USDC on Ethereum. The ad lifecycle moves through several states (`ACTIVE`, `PAUSED`, `EXHAUSTED`, `CLOSED`) as the Maker manages their liquidity.
